...
const SearchBar = () => {
    ...
    return (
        <div className='flex justify-center items-center px-3'>
            <form className='flex justify-center items-center w-full md:w-3/4 p-2' onSubmit={e => e.preventDefault()}>
                <input type="text" name="search" ref={userInput} className='w-full md:w-3/4 px-4 md:px-6 py-2 shadow-black shadow-md outline-none rounded-md text-black' placeholder={lang[language].gptSearchPlaceholder} />
                <button className='px-6 py-2 mx-2 bg-red-600 text-white rounded-md outline-none cursor-pointer shadow-black shadow-md' onClick={handleChangeSearch}> {lang[language].search} </button>
            </form>
        </div>
    )
}
